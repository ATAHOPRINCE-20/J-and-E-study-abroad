...
export function OtherDestinations({ currentCountry }: OtherDestinationsProps) {
  const filtered = allDestinations
    .filter((d) => d.name.toLowerCase() !== currentCountry.toLowerCase())
    .slice(0, 3);
    ...
    <section className="py-32 bg-white">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div>
            <span className="text-[#2563EB] text-xs font-bold uppercase tracking-[0.2em] mb-4 block">Expand Your Horizons</span>
            <h2 className="text-4xl sm:text-5xl font-extrabold text-gray-900 leading-tight">Other Destinations</h2>
          </div>
          <Link to="/destinations" className="group flex items-center gap-3 text-gray-900 font-bold text-lg hover:text-[#2563EB] transition-colors">
            View All Countries
            <div className="w-12 h-12 rounded-full border border-gray-200 flex items-center justify-center group-hover:border-[#2563EB] group-hover:bg-[#2563EB] group-hover:text-white transition-all">
              <ArrowRight size={20} />
            </div>
          </Link>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((dest) => (
            <Link key={dest.name} to={dest.path} className="group relative block overflow-hidden rounded-[32px] bg-gray-100 aspect-[4/5] min-h-[450px]">
              {/* Image & Overlays */}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                <div className="flex justify-between items-end">
                  <div>
                    ...
                    <p className="text-white/70 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-500">Explore Programs</p>
                  </div>
